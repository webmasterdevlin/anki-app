...
function English() {
  ...
  const handleAnswer = () => {
    ...
    if (inputValue.toLowerCase().trim() === currentQuestion.norwegian.toLowerCase()) {
      ...
      alert(`Incorrect! The correct answer was: ${currentQuestion.norwegian}`); // Adjusted for English
  ...
  return (
    <div className="flex h-screen flex-col items-center justify-center bg-gray-100">
      <form onSubmit={handleFormSubmit} className="mb-4 rounded bg-white px-8 pb-8 pt-6 shadow-md">
        <h2 className="mb-2 block text-lg font-bold text-gray-700">Translate to Norwegian:</h2>
        <p className="text-md mb-4 block text-gray-700">{currentQuestion?.english}</p> {/* Adjusted for English */}
        <input
          type="text"
          value={inputValue}
