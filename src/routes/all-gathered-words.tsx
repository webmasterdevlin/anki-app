      ...
      <div className="mb-6">
        <div className="relative">
          <input
            ...
            className="w-full rounded-lg border border-gray-300 p-3 pl-10 focus:border-emerald-500 focus:outline-none focus:ring focus:ring-emerald-200"
          />
          ...
          {filteredWords.map((word, index) => (
            ...
            >
              <div className="mb-2 font-bold text-emerald-700">
                {currentLanguage === 'english' ? word.english : 
                 currentLanguage === 'norwegian' ? word.norwegian : word.spanish}
              </div>
              <div className="text-sm text-gray-600">
                {currentLanguage !== 'english' && <div><span className="font-medium">English:</span> {word.english}</div>}
                {currentLanguage !== 'norwegian' && <div><span className="font-medium">Norwegian:</span> {word.norwegian}</div>}
                {currentLanguage !== 'spanish' && <div><span className="font-medium">Spanish:</span> {word.spanish}</div>}
              </div>
            </div>
          ))}
        </div>
